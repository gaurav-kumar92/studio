...
export const useTextHandler = ({
  ...
  const handleAddOrUpdateText = useCallback((config: any) => {
    ...
    const allAttrs = {
      ...
      'data-gradient-direction': dataAttrs.gradientDirection || 'top-to-bottom',
    };
    ...
    if (allAttrs['data-curvature'] > 0) {
      ...
      const tempText = new window.Konva.Text({ text: 'A', fontSize: allAttrs['data-font-size'], fontFamily: allAttrs['data-font-family'] });
      const charHeight = tempText.height();
      const fontStyle = `${allAttrs['data-is-bold'] ? 'bold ' : ''}${allAttrs['data-is-italic'] ? 'italic ' : ''}`.trim();
      const decorations = [
      ...
      for (const char of allAttrs['data-text']) {
        ...
        const charAngularWidth = (charWidth + allAttrs['data-letter-spacing']) / allAttrs['data-radius'];
      ...
      tempText.destroy();
      circularGroup.rotation((circularGroup.rotation() || 0) -(cumulativeAngle * 180) / (2 * Math.PI));
      ...
      const mainText = new window.Konva.Text({
        ...
        fill: allAttrs['data-solid-color'],
        name: 'text',
        fontStyle: `${allAttrs['data-is-bold'] ? 'bold ' : ''}${allAttrs['data-is-italic'] ? 'italic ' : ''}`.trim(),
        textDecoration: [
          allAttrs['data-is-underline'] ? 'underline' : '',
          allAttrs['data-is-strikethrough'] ? 'line-through' : '',
        ].filter(Boolean).join(' '),
      ...
      if (allAttrs['data-is-shadow']) {
        ...
        mainText.shadowOffset({ x: allAttrs['data-shadow-distance'], y: allAttrs['data-shadow-distance'] });
